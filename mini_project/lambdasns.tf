...
resource "aws_lambda_function" "test_lambda1" {
  ...
  handler       = "lambdasns.notifi"
  source_code_hash = "${base64sha256(file("lambdasns.py"))}"
  runtime = "python3.8"
...
# Adding S3 bucket as trigger to my lambda and giving the permissions
 /* resource "aws_s3_bucket_notification" "aws-lambda-trigger1" {
  bucket = "lambda-sns-bucket"
  lambda_function {
    lambda_function_arn = aws_lambda_function.step_lambda.arn
    events              = ["s3:ObjectCreated:*", "s3:ObjectRemoved:*"]
  }
}*/
  resource "aws_lambda_permission" "step_function_invoke" {
